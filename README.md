...
docs/js/main.js      nav, scroll reveals, hero parallax, counters, gallery,
                     lightbox, map facade, enquiry form (all progressive
                     enhancement). The rest of the scroll-linked motion is CSS
                     scroll-driven animation in main.css — no JS, no cost
docs/assets/*.jpg|webp  client renders, prepped to 800/1200/1600 px via
                     packages/pipeline/src/prep-images.mjs (--widths/--webp)
docs/assets/*.svg|png   floor plans (1BR/2BR only — no penthouse), favicon set
docs/assets/logo-lockup-light.png  the client's official lockup, knocked out of
                     its white background and recoloured for the dark header and
                     footer. Generated by logo-assets.mjs — edit that, not this
docs/assets/nomad-twin-towers-film-2026-08.mp4  client film, 5:51 and 20 MB.
docs/assets/nomad-twin-towers-film-2026-08-av1.mp4  the same film in AV1, listed
                     FIRST in the <video> so capable browsers take it — same
                     bytes, much better picture at the ~0.5 Mbps the 25 MiB cap
                     forces. Keep both: the H.264 is the fallback and the
                     download target. Re-encode BOTH if the film changes
docs/assets/nomad-twin-towers-brochure-2026-08.pdf  the client's brochure,
                     6.1 MB and 14 pages. The three large files on the site, and
                     none is on the critical path: the film is preload="none"
                     behind a poster (and only ONE of the two encodes is ever
                     fetched, whichever the browser picks), the brochure only
                     moves on a download click. See brief.md before changing.
                     The date in the filename is load-bearing for BOTH —
                     /assets/* is cached a week, so a NEW film or brochure needs
                     a NEW name or edges that already hold the old one keep
                     serving it. Client video masters are hundreds of MB and are
                     gitignored at the repo root; only the encode is committed,
                     and it must stay under Cloudflare Pages' 25 MiB file cap
...
hero-scrim.md        how the hero scrim's numbers were measured, and the two
                     ways of measuring them that give confidently wrong
                     answers. Read before touching --hero-scrim
hero-scrim-audit.js  the script that does it — paste into the console on a
                     served copy. Internal, NOT served
logo-assets.mjs      regenerates the lockup and the whole favicon set from the
                     client's official logo. `node logo-assets.mjs`. Internal
```
...
- **Favicon & logo** — **RESOLVED 2026-08-01: the client sent the official
  logo** (`reference/logo-official-2026-08-01.jpeg`), and everything brand-side
  now derives from it via `logo-assets.mjs`. The header and footer carry the
  full lockup; the favicon set is cut from the twin-towers device inside the
  "A". Both stand-ins are gone — the "NT" monogram crop from the price list
  (`logo-mark.png`, deleted) and the typeset NOMAD / TWIN TOWERS that sat beside
  it. The source is a JPEG on white, so the assets are knocked out of that
  background programmatically; **ask the client for the vector original if the
  chance comes up**, and re-run the script against it.
